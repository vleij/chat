 ...
 * @since 1.1
 */
var _getTimeStringAutoShort2 = function(timestamp, mustIncludeTime, uid=''){
    ...
    if(currentYear == srcYear) {
        ...
        // 相差时间（单位：毫秒）
        var on_message_time = get_Storage(uid)
        if(!on_message_time){
            on_message_time = currentTimestamp
        }
        var deltaTime = Math.abs((srcTimestamp - on_message_time));
        ...
        if(currentMonth == srcMonth && currentDateD == srcDateD) {
            ...
            if(deltaTime < 60 * 1000) {
                ...
                if (hours == 12) var str = "中午";
                else str = hour[parseInt(hours / 6)];
                //var timeValue = "" + ((hours >= 12) ? "下午 " : "上午 ")
                ret = str + _formatDate(srcDate, "hh:mm");
            }
        }
